...
class PostgresTraceAnalysisService:
    ...
    def store_extraction_job(self, job: ExtractionJob) -> str:
        ...
        Note:
                        - duration_seconds and extraction_rate are computed by the
                            ExtractionJob model on read
        """
    ...
    def update_extraction_job_status(
        ...
        Side Effects:
            - No cache invalidation (jobs are not cached)
            - Commits transaction automatically
                        - duration_seconds and extraction_rate will reflect the updated
                            values via ExtractionJob properties
