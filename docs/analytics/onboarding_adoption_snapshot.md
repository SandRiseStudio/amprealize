...
This snapshot captures current onboarding and adoption health across Amprealize surfaces. Metrics feed the Milestone Zero dashboard and seed telemetry events so live runs can update KPI progress toward PRD targets (70% behavior reuse, 30% token savings, 80% task completion, 95% compliance coverage).
