...
import {
	AmprealizeClient,
	ProjectSettings as ClientProjectSettings,
	LLMCredential as ClientLLMCredential,
	GitHubLink,
	GitHubResolution,
	GitHubCredential,
	GitHubAppInstallation
} from '../client/AmprealizeClient';
import {
	McpClient,
...
// Extend client interface with local fields
export interface ProjectSettings extends ClientProjectSettings {
	project_id?: string;  // Maps to 'id' in the backend
	local_path?: string;
...
export class ProjectSettingsPanel {
	...
	public static readonly viewType = 'amprealize.projectSettings';

	private readonly _panel: vscode.WebviewPanel;
	private readonly _extensionUri: vscode.Uri;
	private readonly _client: AmprealizeClient;
	private readonly _mcpClient: McpClient | null;
	private _disposables: vscode.Disposable[] = [];
	private _projectId: string;
	private _settings: ProjectSettings | null = null;
	private _bootstrapStatus: BootstrapStatusResult | null = null;
	private _validatedGithub: GitHubValidationResult | null = null;
	private _credentials: LLMCredential[] = [];
	// GitHub credential linking
	private _githubLink: GitHubLink | null = null;
	private _githubResolution: GitHubResolution | null = null;
	private _myGitHubCredentials: GitHubCredential[] = [];
	private _myGitHubAppInstallations: GitHubAppInstallation[] = [];

	private constructor(
		panel: vscode.WebviewPanel,
		extensionUri: vscode.Uri,
		client: AmprealizeClient,
		projectId: string,
		mcpClient?: McpClient
	) {
		this._panel = panel;
			...
			async (message) => {
				switch (message.type) {
					...
					case 'unlinkGitHub':
						...
						return;
					case 'openOnboarding':
						vscode.commands.executeCommand('amprealize.openOnboarding');
						return;
				}
			},
			null,
	...
	private async _loadBootstrapStatus() {
		if (!this._mcpClient) {
			return;
		}
		try {
	...
	private async _addCredential(provider: string, apiKey: string, name?: string) {
		try {
	...
	private async _deleteCredential(credentialId: string) {
	...
	private async _reEnableCredential(credentialId: string, apiKey: string) {
		try {
	...
	private async _linkGitHubPAT(token: string, name?: string) {
		try {
	...
	private async _linkGitHubApp(installationId: number) {
		try {
	...
	private async _unlinkGitHub(linkType?: 'pat' | 'app') {
		const confirm = await vscode.window.showWarningMessage(
			'Are you sure you want to unlink this GitHub credential?',
			{ modal: true },
			'Unlink'
		);
		if (confirm !== 'Unlink') {
	...
	private _renderBootstrapSection(): string {
		const status = this._bootstrapStatus;
		if (!this._mcpClient) {
			return '';
		}

		const profileLabels: Record<WorkspaceProfile, string> = {
			'solo-dev': 'Solo Developer',
			'amprealize-platform': 'Amprealize Platform',
		...
		const profileLabel = profileLabels[status.profile as WorkspaceProfile] || status.profile || 'Unknown';
		return `
		<section class="settings-section">
			<h2>Workspace Profile</h2>
			<p class="section-description">
	...
	private _renderGitHubResolutionStatus(): string {
	...
	private _renderGitHubLinkSection(): string {
			...
			<div class="github-link-form">
				<div class="link-option">
					...
					${appOptions
						...
						`
						: `<p class="empty-state">No GitHub App installations available. <a href="https://github.com/apps/amprealize" target="_blank">Install the GitHub App</a></p>`
					}
				</div>
