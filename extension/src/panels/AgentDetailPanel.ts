...
import { Agent, AgentStatus, AmprealizeClient } from '../client/AmprealizeClient';
...
export class AgentDetailPanel {
	...
	public static readonly viewType = 'amprealize.agentDetail';
	...
	private constructor(panel: vscode.WebviewPanel, extensionUri: vscode.Uri, client: AmprealizeClient) {
		this._panel = panel;
	...
	public static async createOrShow(extensionUri: vscode.Uri, client: AmprealizeClient, agent: Agent) {
	...
	public static revive(panel: vscode.WebviewPanel, extensionUri: vscode.Uri, client: AmprealizeClient) {
		AgentDetailPanel.currentPanel = new AgentDetailPanel(panel, extensionUri, client);
	}
	...
	private async _editAgent() {
		...
		if (selected.value === 'system_prompt') {
			...
			vscode.window.showInformationMessage('Edit the system prompt and use "amprealize.updateAgent" command to save');
			return;
