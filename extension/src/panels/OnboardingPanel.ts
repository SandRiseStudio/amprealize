/**
 * Onboarding Panel (AMPREALIZE-276 / E2 Phase 3)
 *
...
import {
	McpClient,
	WorkspaceProfile,
	BootstrapDetectResult,
	BootstrapStatusResult,
	BootstrapInitResult
} from '../client/McpClient';

type OnboardingStep = 'detect' | 'confirm' | 'init' | 'complete';
...
const PROFILE_DESCRIPTIONS: Record<WorkspaceProfile, string> = {
	'solo-dev': 'Individual developer workspace with minimal process overhead.',
	'amprealize-platform': 'Amprealize core platform development (behaviors, MCP, services).',
...
const PROFILE_ICONS: Record<WorkspaceProfile, string> = {
	'solo-dev': '👤',
	'amprealize-platform': '🏗️',
	'team-collab': '👥',
	'extension-dev': '🧩',
	'api-backend': '⚙️',
...
export class OnboardingPanel {
	...
	public static readonly viewType = 'amprealize.onboarding';
	...
	// State
	private _step: OnboardingStep = 'detect';
	private _detecting = false;
	private _initializing = false;
	private _detection: BootstrapDetectResult | null = null;
	private _status: BootstrapStatusResult | null = null;
	private _initResult: BootstrapInitResult | null = null;
	private _selectedProfile: WorkspaceProfile | null = null;
			...
			async (message) => {
				switch (message.type) {
					case 'detect':
						...
						return;
					case 'selectProfile':
						this._selectedProfile = message.profile as WorkspaceProfile;
						this._step = 'confirm';
						this.update();
						return;
					case 'confirmProfile':
						await this.initWorkspace(message.profile as WorkspaceProfile);
						return;
					case 'changeProfile':
						this._step = 'confirm';
						this.update();
	 ...
	 */
	public static async checkAndPrompt(extensionUri: vscode.Uri, mcpClient: McpClient): Promise<void> {
		try {
			const status = await mcpClient.bootstrapStatus();
			if (!status.is_bootstrapped) {
				const action = await vscode.window.showInformationMessage(
					'This workspace has not been set up with Amprealize. Would you like to configure it now?',
					'Set Up Workspace',
					'Not Now'
				);
	...
	private async initWorkspace(profile: WorkspaceProfile) {
	...
	private getHtmlForWebview(webview: vscode.Webview): string {
		const styleUri = webview.asWebviewUri(
			vscode.Uri.joinPath(this._extensionUri, 'src', 'styles', 'OnboardingPanel.css')
		);
...
<head>
	...
	<title>Amprealize Workspace Setup</title>
	<link href="${styleUri}" rel="stylesheet">
</head>
<body>
	<div class="onboarding-container">
		<header class="onboarding-header">
			<h1>Amprealize Workspace Setup</h1>
	...
	private renderDetectStep(): string {
		if (this._detecting) {
			return `
	...
	private renderConfirmStep(): string {
		...
		const profiles: WorkspaceProfile[] = [
			'solo-dev', 'amprealize-platform', 'team-collab',
