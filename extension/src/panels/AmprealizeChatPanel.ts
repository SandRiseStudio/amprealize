/**
 * Amprealize Chat Panel
 *
 * A native VS Code webview panel for interacting with Amprealize MCP tools directly.
...
export class AmprealizeChatPanel {
	public static currentPanel: AmprealizeChatPanel | undefined;
	public static readonly viewType = 'amprealize.chat';
	...
	private _toolGroups: ToolGroup[] = [];
	private _activeTools: ActiveToolInfo[] = [];
	...
	public static createOrShow(
		...
		if (AmprealizeChatPanel.currentPanel) {
			AmprealizeChatPanel.currentPanel._panel.reveal(column);
			return;
		}

		const panel = vscode.window.createWebviewPanel(
			AmprealizeChatPanel.viewType,
			'Amprealize Chat',
			column,
			{
				enableScripts: true,
		...
		AmprealizeChatPanel.currentPanel = new AmprealizeChatPanel(panel, extensionUri, mcpClient);
	}
	...
	public dispose() {
		AmprealizeChatPanel.currentPanel = undefined;
		this._panel.dispose();
...
@behaviors.getForTask {"task_description": "..."} - Get behaviors for a task`);
	...
	private async _handleToolCommand(toolName: string, argsText: string) {
...
Or type /tools to see available tools.`;
	...
	private async _loadToolGroups() {
		try {
			const result = await this._mcpClient.callTool<{ groups: ToolGroup[] }>('tools.listGroups', {});
	...
	private async _activateToolGroup(groupId: string) {
		try {
			const result = await this._mcpClient.callTool<{ loaded_tools: string[] }>('tools.activateGroup', {
				group_id: groupId
			});
	...
	private async _deactivateToolGroup(groupId: string) {
		try {
	...
	private async _listActiveTools() {
		try {
			// Use tools/list to get active tools
			const result = await this._mcpClient.callTool<{ tools: ActiveToolInfo[] }>('tools.activeGroups', {});
	...
	private _addAssistantMessage(content: string, isError = false) {
		...
		<div class="main-content">
			<div class="messages" id="messages">
				<div class="message system">
					Welcome to Amprealize Chat! This panel connects directly to the MCP server.<br>
