...
exports.PlanComposerPanel = void 0;
const vscode = __importStar(require("vscode"));
class PlanComposerPanel {
    constructor(panel, client, templateId) {
        this.client = client;
    ...
    static async createOrShow(extensionUri, client, template) {
        ...
        // Create new panel
        const panel = vscode.window.createWebviewPanel('planComposer', 'Plan Composer', column || vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    getWebviewContent() {
        const workflowOptions = this.workflows.map(w => `<option value="${w.template_id}" ${w.template_id === this.templateId ? 'selected' : ''}>
                ${this.escapeHtml(w.name)} (${w.role_focus || w.role})
            </option>`).join('');
        const behaviorOptions = this.behaviors.map(b => `<option value="${b.behavior_id}">${this.escapeHtml(b.name)}</option>`).join('');
        return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    <div id="workflow-details" style="display: none;">
        ...
        <div class="behavior-injection">
            ...
            <div class="form-group">
                ...
                <p class="info-text">Amprealize will retrieve relevant behaviors using the BCI retriever.</p>
            </div>
    ...
    async handleMessage(message) {
        ...
        switch (message.command) {
            case 'runWorkflow':
                try {
                    const behaviors = Array.isArray(message.behaviors) ? message.behaviors : [];
                    const behaviorDetails = Array.isArray(message.behaviorDetails)
                    ...
                    const result = await this.client.runWorkflow(message.workflowId, message.context, { source: 'plan_composer.run' });
                    vscode.window.showInformationMessage(`Workflow started! Run ID: ${result.run_id}`, 'View Status').then(selection => {
                        if (selection === 'View Status') {
                            vscode.commands.executeCommand('amprealize.viewWorkflowStatus', result.run_id);
                        }
                    });
                }
                catch (error) {
    ...
    async handleBCIRetrieve(message) {
        ...
        if (Array.isArray(message.tags)) {
            options.tags = message.tags.filter((tag) => typeof tag === 'string');
        }
        ...
        try {
            const response = await this.client.bciRetrieve(options);
    ...
    async handleBCIValidate(message) {
        const requestId = typeof message.requestId === 'string' ? message.requestId : undefined;
        if (!requestId) {
            return;
        }
        ...
        const prepended = prependedPayload
            .map((item) => {
            ...
            const mapped = { behavior_name: behaviorName };
        ...
        try {
            const response = await this.client.bciValidateCitations({
                outputText,
                prepended,
                minimumCitations,
                allowUnlisted,
...
exports.PlanComposerPanel = PlanComposerPanel;
//# sourceMappingURL=PlanComposerPanel.js.map
