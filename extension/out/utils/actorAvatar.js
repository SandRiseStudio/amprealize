...
exports.createActorViewModel = createActorViewModel;
exports.buildActorAvatarSvg = buildActorAvatarSvg;
exports.buildActorAvatarDataUri = buildActorAvatarDataUri;
exports.buildActorAvatarHtml = buildActorAvatarHtml;
...
const PERSONAS_BY_KIND = {
    human: ['human'],
    agent: ['robot', 'cat', 'dog', 'fox', 'owl'],
    service: ['robot'],
};
const PALETTES_BY_PERSONA = {
...
function pick(items, hash, shift) {
    return items[(hash >>> shift) % items.length];
}
function generateAvatarVariant(seed, kind, styleVersion = 1) {
...
function createActorViewModel(input) {
...
function buildActorAvatarSvg(actor, size = 40) {
...
function buildActorAvatarDataUri(actor, size = 40) {
    return `data:image/svg+xml;utf8,${encodeURIComponent(buildActorAvatarSvg(actor, size))}`;
}
function buildActorAvatarHtml(actor, size = 44) {
...
//# sourceMappingURL=actorAvatar.js.map
